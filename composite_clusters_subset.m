...
siteName = 'HAT_A_05';
inFileName = 'D:\HAT05A_metadata\TPWS\ClusterOct2016\HAT_A_05_disk16a_Delphin_clusters_PG0_PR95_MIN25_MOD0_noFP.mat';
...
saveOutputMat = 0; % Set to 1 if you want to save final output mat file
%% 
...
if size(sumSpec,1)>maxClust
    excludedIn = 1:round(size(sumSpec,1)/maxClust):size(sumSpec,1);
    % excludedIn = sort(randperm(size(sumSpec,1),maxClust));
...
sumSpecMat = vertcat(sumSpec{excludedIn});
...
cRateMat = vertcat(clickRate{excludedIn});
...
[nodeSet,excludedOut,rankAssign] = cluster_nodes(compDist,...
